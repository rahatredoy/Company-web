 ...
 * cookie, admin password and card-holder detail would cross the wire in the
 * clear with nothing in the logs to say so. This is the control plane: the
 * traffic it carries is billing, provisioning and the platform's single
 * super-admin session.
 *
 ...
 *
 * `/health` is exempt because a load balancer probes it over plain http on the
 * instance's own address, before any TLS listener is involved, and a liveness
 * check that fails on transport takes healthy instances out of rotation.
 *
 * `/api/v1/internal` is **not** exempt. It is the one path `client-api` calls,
 * it carries `x-internal-key` — a shared secret good for every tenant lookup on
 * the platform — and the two APIs are separately deployed, so "it is internal"
 * describes the audience and not the network between them.
 *
 * A deliberate copy of `client-api/src/plugins/https.ts`; the two APIs deploy
 * independently and share no package.
 */
