 ...
 *
 * The labels are dictionary keys, translated where they are drawn.
 *
...
export const DASHBOARD_GRANULARITIES: { key: DashboardGranularity; label: MessageKey }[] = [
  { key: 'day', label: 'Daily' },
 ...
 *
 * In the panel's language (`locale`, from `t.locale`), and in the **store's**
 * timezone, which the API reports alongside the window it used. Formatting in
 * the reader's own zone instead would put a date on the label that the figures
 * underneath it were not computed for — a shop in Dhaka read from London would
 * be told its week ended a day later than it did.
 * Server and browser therefore agree, which is also what keeps hydration quiet.
 */
export function formatDashboardRange(from: string, to: string, timeZone: string, locale = 'en-US'): string {
