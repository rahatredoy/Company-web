...
export function layout(storeName: string, heading: string, bodyHtml: string): string {
  return `<!doctype html>
<html><body style="margin:0;padding:24px;background:#f4f5f7;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#1f2937;">
  ...
  <p style="max-width:560px;margin:16px auto 0;font-size:12px;color:#9ca3af;text-align:center;">
    Sent by ${escapeHtml(storeName)} via ${escapeHtml(config.mail.fromName)}.
  </p>
</body></html>`;
}
...
export function button(label: string, url: string): string {
  ...
  <p style="margin:0 0 8px;font-size:13px;color:#6b7280;">If the button does not work, paste this link into your browser:</p>
